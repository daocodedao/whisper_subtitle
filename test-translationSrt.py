...
#             print(
#                 f"{sub.index}\n"
#                 f"{format_timestamp(sub.start, always_include_hours=True)} --> "
#                 f"{format_timestamp(sub.end, always_include_hours=True)}\n"
#                 f"{translation}",
#                 file=srt,
#                 flush=True,
#             )
...
with open(outPath, "w", encoding="utf-8") as outFile:
    with open('./sample/simple5.srt', 'r') as srcFile:
        ...
        for sub in subs:
            ...
            print(
                f"{sub.index}\n"
                f"{format_timestamp(sub.start.seconds, always_include_hours=True)} --> "
                f"{format_timestamp(sub.end.seconds, always_include_hours=True)}\n"
                f"{translation}",
                file=outFile,
                flush=True,
            )
